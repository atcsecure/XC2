...
bool error(const char *format, ...)
{
    ...
    va_end(arg_ptr);
    printf("ERROR: %s\n", str.c_str());
   // std::cerr << " THIS ERROR IS IMPORTANT: " << str.c_str();
    return false;
}
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "XCurrencyd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong XCurrency will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("XCurrency"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
