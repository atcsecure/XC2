...
{
    resize(850, 550);
    QString title = tr("XCurrency") + " - " + tr("Wallet");
    if (GetBoolArg("-distmix-autonode"))
    {
        title = title + QString(" ") + tr("[autonode]");
    }
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a XCurrency address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About XCurrency"), this);
    aboutAction->setToolTip(tr("Show information about XCurrency"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for XCurrency"));
    ...
    VERIFY(connect(quitAction,             SIGNAL(triggered()),     qApp, SLOT(quit())));
    VERIFY(connect(aboutAction,            SIGNAL(triggered()),     this, SLOT(aboutClicked())));
    VERIFY(connect(aboutQtAction,          SIGNAL(triggered()),     qApp, SLOT(aboutQt())));
    VERIFY(connect(optionsAction,          SIGNAL(triggered()),     this, SLOT(optionsClicked())));
    VERIFY(connect(toggleHideAction,       SIGNAL(triggered()),     this, SLOT(toggleHidden())));
    VERIFY(connect(encryptWalletAction,    SIGNAL(triggered(bool)), this, SLOT(encryptWallet(bool))));
    VERIFY(connect(backupWalletAction,     SIGNAL(triggered()),     this, SLOT(backupWallet())));
    VERIFY(connect(changePassphraseAction, SIGNAL(triggered()),     this, SLOT(changePassphrase())));
    VERIFY(connect(lockWalletToggleAction, SIGNAL(triggered()),     this, SLOT(lockWalletToggle())));
    VERIFY(connect(signMessageAction,      SIGNAL(triggered()),     this, SLOT(gotoSignMessageTab())));
    VERIFY(connect(verifyMessageAction,    SIGNAL(triggered()),     this, SLOT(gotoVerifyMessageTab())));
}
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("XCurrency client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("XCurrency client"));
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid XCurrency address or malformed URI parameters."));
    }
