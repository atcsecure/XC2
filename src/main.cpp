...
const string strMessageMagic = "XCurrency Signed Message:\n";
...
bool CTransaction::IsStandard() const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        if (txin.scriptSig.size() > 500)
            return false;
        //std::cerr << "never gonna give up" << std::endl;
                        ...
                        bool* pfMissingInputs)
{
    ...
    if (/*!fTestNet &&*/ !tx.IsStandard()) // HACK: hax
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in XCurrency
    bool fStrictPayToScriptHash = true; // Always active in XCurrency
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "XCurrency", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
//if ((md.amount + 5. < balance.get_real())& ( balance.get_real()> 999) ) {
    ...
    else if (strCommand == "mssearch")
    {
        bool isAutonode = GetBoolArg("-distmix-autonode");

        CMultisigSearch msgsearch;

        vRecv >> msgsearch.version;
        if (msgsearch.version != CMultisigTx::version())
        {
            std::cerr << "incorrect version, mssearch ignored" << std::endl
                      << "received: " << msgsearch.version
                      << " , local: " << CMultisigTx::version() << std::endl;
            return true;
        }

        vRecv >> msgsearch;

        if (msgtx == NULL && isAutonode)
        {
            static int errorCounter = 0;

            std::cerr << "sssearch handler, distmix autonode" << std::endl;

            std::vector<std::string> addrs = getLocalBitcoinAddresses();

            try
            {
                Array params;
                params.push_back(Value(addrs[0]));
                params.push_back(Value(static_cast<double>(msgsearch.amount)/COIN));
                distmix(params, false);
            }
            // catch (Object &)
            catch (...)
            {
                ...
                // std::cerr << e.what();
                std::cerr << "exception encountered, distmix autojoin not processed" << std::endl;
                ...
                if (errorCounter >= 8)
                {
                    // too many errors, disable distmix auto processing
                    std::cerr << "distmix autonode disabled, too many errors" << std::endl;
                    mapArgs["-distmix-autonode"] = "0";
                }
            }
        }
        ...
        if (msgtx != NULL)
        {
            std::cerr << "Start mssearch handler" << std::endl;

            {
                LOCK(cs_msgtx);

                if (!msgtx->joined && !msgtx->joining)
                {
                    if (msgtx->amount != msgsearch.amount)
                    {
                        std::cerr << "Amount does not match; ignoring mssearch" << std::endl;
                    ...
                    else
                    {
                        ...
                        CMultisigJoin msgjoin;
                        msgjoin.amount = msgsearch.amount;
                        msgtx->joining = true;
                        pfrom->PushMessage("msjoin", CMultisigTx::version(), msgjoin);
                    }
                }
            }
        }
    ...
    else if (strCommand == "msjoin" && msgtx != NULL)
    {
        std::cerr << "Start msjoin handler" << std::endl;

        CMultisigJoin msgjoin;

        vRecv >> msgjoin.version;
        if (msgjoin.version != CMultisigTx::version())
        {
            std::cerr << "incorrect version, msjoin ignored" << std::endl
                      << "received: " << msgjoin.version
                      << " , local: " << CMultisigTx::version() << std::endl;
            return true;
        }
...

        {
            ...
            if (msgjoin.amount != msgtx->amount)
            {
                std::cerr << "Amount does not match; ignoring msjoin" << std::endl;
            }
            else
            {
                if (msgtx->root)
                {
                    CMultisigAccept msgaccept;

                    // to ensure it only pushes a message to the first that sends msjoin
                    msgtx->root         = false;
                    msgtx->next         = pfrom;
                    msgaccept.start     = to_simple_string(msgtx->start);
                    std::random_shuffle(msgtx->outputs.begin(), msgtx->outputs.end());
                    msgaccept.outputs   = msgtx->outputs;
                    msgaccept.inputs    = msgtx->inputs;
                    msgaccept.amount    = msgjoin.amount;
                    msgaccept.fee       = msgtx->fee;
                    msgaccept.autonodes = msgtx->autonodes;

                    std::cerr << "Sending msaccept with "
                              << msgtx->inputs.size() << " inputs." << std::endl;

                    pfrom->PushMessage("msaccept", CMultisigTx::version(), msgaccept);
                }

                else
                {
                    std::cerr << "Received msjoin but not root" << std::endl;

                    CMultisigJoin msrj;
                    msrj.amount = msgjoin.amount;

                    // another host already joined, send reject
                    pfrom->PushMessage("msreject", CMultisigTx::version(), msrj);
                }
            }
        }
    }

    else if (strCommand == "msaccept" && msgtx != NULL)
    {
        std::cerr << "Start msaccept handler" << std::endl;

        CMultisigAccept msgaccept;

        vRecv >> msgaccept.version;
        if (msgaccept.version != CMultisigTx::version())
        {
            std::cerr << "incorrect version, msaccept ignored" << std::endl
                      << "received: " << msgaccept.version
                      << " , local: " << CMultisigTx::version() << std::endl;
            return true;
        }
...

        {
            ...
            if (msgtx->amount != msgaccept.amount)
            {
                std::cerr << "AMOUNT OF " << msgaccept.amount
                          << " DOES NOT MATCH " << msgtx->amount << std::endl;
                pfrom->PushMessage("msfailure");
            }

            else
            {
                std::cerr << "becoming root" << std::endl;

                msgtx->root    = true;
                msgtx->joined  = true;
                msgtx->joining = false;
                msgtx->prev    = pfrom;
                msgtx->start   = time_from_string(msgaccept.start);
                msgtx->outputs = msgaccept.outputs;
                msgtx->inputs  = msgaccept.inputs;
                msgtx->fee    += msgaccept.fee;

                // add received autonodes list
                msgtx->autonodes.insert(msgtx->autonodes.end(), msgaccept.autonodes.begin(), msgaccept.autonodes.end());

                std::cerr << "Old number of inputs: " << msgtx->inputs.size() << std::endl;
                msgtx->inputs.insert(msgtx->inputs.end(), msgtx->myinputs.begin(), msgtx->myinputs.end());
                std::cerr << "New number of inputs: " << msgtx->inputs.size() << std::endl;

                std::cerr << "Old number of outputs: " << msgtx->outputs.size() << std::endl;
                msgtx->outputs.insert(msgtx->outputs.end(), msgtx->myoutputs.begin(), msgtx->myoutputs.end());
                std::cerr << "New number of outputs: " << msgtx->outputs.size() << std::endl;
            }
        }
    }

    else if (strCommand == "msreject" && msgtx != NULL)
    {
        std::cerr << "Start msreject handler" << std::endl;

        CMultisigJoin msrj;

        vRecv >> msrj.version;
        if (msrj.version != CMultisigTx::version())
        {
            std::cerr << "incorrect version, msreject ignored" << std::endl
                      << "received: " << msrj.version
                      << " , local: " << CMultisigTx::version() << std::endl;
            return true;
        }
...

        {
            ...
            if (msrj.amount != msgtx->amount)
            {
                std::cerr << "Amount does not match; ignoring msreject" << std::endl;
            }
            else
            {
                delete msgtx;
        ...
        uiInterface.NotifyDistmixPaymentStatusChanged("msreject");
    }

    else if (strCommand == "mssign" && msgtx != NULL)
    {
        std::cerr << "Start mssign handler" << std::endl;

        CMultisigSign msgsign;

        vRecv >> msgsign.version;
        if (msgsign.version != CMultisigTx::version())
        {
            std::cerr << "incorrect version, mssign ignored" << std::endl
                      << "received: " << msgsign.version
                      << " , local: " << CMultisigTx::version() << std::endl;
            return true;
        }
        ...
        std::string uistatus = "mssuccess";

        {
            LOCK(cs_msgtx);

            if (msgsign.amount != msgtx->amount)
            {
                std::cerr << "Amount does not match; ignoring mssign" << std::endl;
                uistatus = "msfailure";
            }
            else
            {
                // sign transaction
                std::cerr << "signing transaction" << std::endl;

                Array params;
                params.push_back(msgsign.rawtx);

                Object result = signrawtransaction(params, false).get_obj();

                std::cerr << "transaction signed" << std::endl;

                CMultisigSign msgsignout;
                msgsignout.amount = msgsign.amount;
                msgsignout.rawtx = result[0].value_.get_str();

                if (msgtx->prev != NULL)
                {
                    msgtx->prev->PushMessage("mssign", CMultisigTx::version(), msgsignout);

                }

                else if (result[1].value_.get_bool())
                {
                    // send transaction
                    std::cerr << "Sending transaction to network." << std::endl;

                    params.clear();
                    params.push_back(msgsignout.rawtx);

                    try
                    {
                        sendrawtransaction(params, false);

                        msgtx->next->PushMessage("mssuccess");
                ...
                {
                    std::cerr << "ALL INPUTS NOT SIGNED, ABORTING" << std::endl;
                    uistatus = "msfailure";
                }
            }
        }

        uiInterface.NotifyDistmixPaymentStatusChanged(uistatus);
    }

    else if (strCommand == "mssuccess" && msgtx != NULL)
    {
        std::cerr << "Start mssuccess handler" << std::endl;

        {
            ...
            std::cout << "COMPLETED MULTISIG TRANSACTION" << std::endl;
        ...
        uiInterface.NotifyDistmixPaymentStatusChanged("mssuccess");
    }

    else if (strCommand == "msfailure" && msgtx != NULL)
    {
        std::cerr << "Start msfailure handler" << std::endl;

        {
            ...
            std::cerr << "FAILED TO COMPLETE MULTISIG TRANSACTION" << std::endl;
